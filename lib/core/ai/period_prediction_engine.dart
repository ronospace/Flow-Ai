...
class PeriodPredictionEngine {
  static final PeriodPredictionEngine _instance =
  ...
  // ignore: unused_field
  static const int _minimumDataPoints =
      3; // Need at least 3 cycles for prediction
  // ignore: unused_field
  ...
  Future<PeriodPrediction> predictNextPeriod(String userId) async {
    try {
      debugPrint(
        '🧠 AI Engine: Starting advanced period prediction for user $userId',
      );
      ...
      finalPrediction.insights = await _generateAIInsights(
        historicalData,
        finalPrediction,
      );

      debugPrint(
        '🎯 AI Prediction complete: Next period ${finalPrediction.nextPeriodDate}, Confidence: ${finalPrediction.confidenceLevel}%',
      );
  ...
  Future<PeriodPrediction> _neuralNetworkPrediction(
    List<CycleData> data,
  ) async {
    ...
    for (
      int i = 0;
      i < patterns.cycleLengths.length && i < _neuralWeights.length;
      i++
    ) {
    ...
    final lastPeriodEnd =
        data.last.endDate ?? data.last.startDate.add(Duration(days: 5));
    final nextPeriodDate = lastPeriodEnd.add(
      Duration(days: cycleLengthPrediction.round()),
    );
  ...
  Future<PeriodPrediction> _bayesianPrediction(List<CycleData> data) async {
    ...
    final lastPeriodEnd =
        data.last.endDate ?? data.last.startDate.add(Duration(days: 5));
    final nextPeriodDate = lastPeriodEnd.add(
      Duration(days: posteriorMean.round()),
    );
  ...
  Future<PeriodPrediction> _fourierSeasonalPrediction(
    List<CycleData> data,
  ) async {
    if (data.length < 6) {
    ...
    final seasonalAdjustment = _calculateSeasonalFromFrequency(
      dominantFrequency,
    );
    final trendComponent = _calculateTrend(timeSeries);

    final baseCycleLength = timeSeries.isNotEmpty
        ? timeSeries.reduce((a, b) => a + b) / timeSeries.length
        : _defaultCycleLength;

    final adjustedCycleLength =
        baseCycleLength * seasonalAdjustment + trendComponent;

    final lastPeriodEnd =
        data.last.endDate ?? data.last.startDate.add(Duration(days: 5));
    final nextPeriodDate = lastPeriodEnd.add(
      Duration(days: adjustedCycleLength.round()),
    );
  ...
  Future<PeriodPrediction> _regressionPrediction(List<CycleData> data) async {
    ...
    final lastPeriodEnd =
        data.last.endDate ?? data.last.startDate.add(Duration(days: 5));
    final nextPeriodDate = lastPeriodEnd.add(
      Duration(days: prediction.round()),
    );
    ...
    return PeriodPrediction(
      ...
      periodLengthPrediction: _defaultPeriodLength.round(),
      confidenceLevel: _calculateRegressionConfidence(
        features,
        targets,
        coefficients,
      ),
  ...
  Future<List<PeriodPrediction>> _runEnsemblePrediction(
    List<CycleData> data,
  ) async {
  ...
  PeriodPrediction _combineEnsemblePredictions(
    List<PeriodPrediction> predictions,
  ) {
    ...
    for (final prediction in predictions) {
      ...
      final daysSinceEpoch =
          prediction.nextPeriodDate.millisecondsSinceEpoch /
          (1000 * 60 * 60 * 24);
    ...
    final averageDate = DateTime.fromMillisecondsSinceEpoch(
      (averageDaysSinceEpoch * 1000 * 60 * 60 * 24).round(),
    );
  ...
  Future<List<String>> _generateAIInsights(
    ...
    if (patterns.isRegular) {
      insights.add(
        '🎯 Your cycle is highly regular! Our AI detected a consistent ${patterns.averageCycleLength.round()}-day pattern.',
      );
    } else {
      insights.add(
        '📊 Our AI detected some variability in your cycle. This is completely normal for ${((1 - patterns.variance) * 100).round()}% of people.',
      );
    }
    ...
    if (prediction.confidenceLevel > 85) {
      insights.add(
        '✨ High Confidence Prediction: Our advanced AI models agree with ${prediction.confidenceLevel}% confidence.',
      );
    } else if (prediction.confidenceLevel > 70) {
      insights.add(
        '📈 Good Prediction: Continue tracking to improve accuracy. Current confidence: ${prediction.confidenceLevel}%',
      );
    } else {
      insights.add(
        '🧠 Learning Mode: Our AI is still learning your unique patterns. More data will improve predictions.',
      );
    }
  ...
  double _bayesianUpdate(
    ...
    final posteriorMean =
        (priorMean / priorVariance + likelihood.mean / likelihood.variance) /
        precision;
  ...
  List<double> _multipleLinearRegression(
    ...
    coefficients[0] =
        targets.reduce((a, b) => a + b) / targets.length; // Intercept
  ...
  int _calculateFourierConfidence(List<Complex> frequencies) {
    ...
    final avgMagnitude =
        frequencies.map((f) => f.magnitude).reduce((a, b) => a + b) /
        frequencies.length;
  ...
  int _calculateRegressionConfidence(
    List<List<double>> features,
    List<double> targets,
  ...
  int _calculateEnsembleConfidence(List<PeriodPrediction> predictions) {
    ...
    final dates = predictions
        .map((p) => p.nextPeriodDate.millisecondsSinceEpoch)
        .toList();
    final avgDate = dates.reduce((a, b) => a + b) / dates.length;
    final dateVariance =
        dates.map((d) => math.pow(d - avgDate, 2)).reduce((a, b) => a + b) /
        dates.length;
    final dateStdDev =
        math.sqrt(dateVariance) / (1000 * 60 * 60 * 24); // Convert to days

    final agreementBonus = dateStdDev < 2
        ? 10
        : 0; // High agreement if predictions within 2 days
  ...
  String _analyzeSymptomPatterns(List<CycleData> data) {
    ...
    final mostCommon = symptomFrequency.entries.reduce(
  ...
  PeriodPrediction _generateInitialPrediction() {
    final nextPeriodDate = DateTime.now().add(
      Duration(days: _defaultCycleLength.round()),
    );
    return PeriodPrediction(
      nextPeriodDate: nextPeriodDate,
