...
class PlatformConfig {
  ...
  Map<String, dynamic> getFirebaseConfig() {
    ...
    if (platformInfo.isWeb) {
      baseConfig.addAll({'measurementId': _getMeasurementId()});
    }

    if (platformInfo.platform == TargetPlatform.iOS) {
      baseConfig.addAll({'iosBundleId': _getiOSBundleId()});
    }

    if (platformInfo.platform == TargetPlatform.android) {
      baseConfig.addAll({'androidPackageName': _getAndroidPackageName()});
    }
  ...
  Map<String, bool> getFeatureFlags() {
    ...
    return {
      ...
      'enablePushNotifications':
          platformInfo.isMobile || platformInfo.isDesktop,
      ...
      'enableGoogleFitIntegration':
          platformInfo.platform == TargetPlatform.android,
    };
  }
  ...
  Map<String, dynamic> getBuildConfig() {
    ...
    return {
      ...
      'enableR8':
          platformInfo.platform == TargetPlatform.android && !kDebugMode,
      'enableBitcode':
          platformInfo.platform == TargetPlatform.iOS && !kDebugMode,
      'enableSplitApk':
          platformInfo.platform == TargetPlatform.android && !kDebugMode,
      'enableAppBundle':
          platformInfo.platform == TargetPlatform.android && !kDebugMode,
      ...
      'targetSdkVersion': platformInfo.platform == TargetPlatform.android
          ? 34
          : null,
      'minSdkVersion': platformInfo.platform == TargetPlatform.android
          ? 21
          : null,
      'deploymentTarget': platformInfo.platform == TargetPlatform.iOS
          ? '12.0'
          : null,
    };
  }
...
class PlatformBuildConfig {
  ...
  static Map<String, List<String>> getCompilerFlags() {
    ...
    if (platformInfo.platform == TargetPlatform.iOS) {
      flags['ios'] = ['--enable-impeller', '--ios-deterministic-rendering'];
    }
