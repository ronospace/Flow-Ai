...
class PersonalizationEngine {
  ...
  Future<void> learnFromCorrection({
    ...
    final correctionHistory =
        _adaptiveLearningModel['correction_history']
    ...
    debugPrint(
      '📊 Learning from correction: $predictionType, accuracy: ${accuracy[correctionKey]?.toStringAsFixed(2)}',
    );
  }
  ...
  Future<Map<String, double>> adaptPredictionModel({
    ...
    adaptations['cycle_length_adjustment'] =
        personalCyclePattern['adjustment'] ?? 0.0;

    // Symptom likelihood adaptation
    final symptomAdaptation = _analyzePersonalSymptomPatterns(cycles, profile);
    adaptations['symptom_likelihood_multiplier'] =
        symptomAdaptation['multiplier'] ?? 1.0;

    // Mood/energy baseline adaptation
    final moodEnergyAdaptation = _analyzePersonalMoodEnergyPatterns(
      cycles,
      profile,
    );
    adaptations['mood_baseline_shift'] =
        moodEnergyAdaptation['mood_shift'] ?? 0.0;
    adaptations['energy_baseline_shift'] =
        moodEnergyAdaptation['energy_shift'] ?? 0.0;

    // Confidence calibration based on user's historical accuracy
    final confidenceAdaptation = _calculatePersonalConfidenceCalibration(
      userId,
    );
  ...
  Future<List<PersonalizedRecommendation>> generatePersonalizedRecommendations({
    ...
    final lifestyleRecs = await _generateLifestyleRecommendations(
      profile,
      currentPhase,
    );
    ...
    recommendations.sort(
      (a, b) => b.personalRelevance.compareTo(a.personalRelevance),
    );
  ...
  Future<void> _initializeUserSpecificModels(UserProfile profile) async {
    ...
    final behaviorModel =
        _behavioralPatterns['tracking_behavior'] as Map<String, dynamic>;
  ...
  Future<void> _adaptModelParameters(
    String userId,
    ...
    final strategies =
        _adaptiveLearningModel['adaptation_strategies'] as Map<String, double>;
      ...
      if (stressImpact > 0.3) {
        insights.add(
          PersonalizedInsight(
            ...
            description:
                'Your cycles tend to be ${stressImpact > 0.5 ? 'significantly' : 'moderately'} '
    ...
    if (consistency < 0.7) {
      insights.add(
        PersonalizedInsight(
          ...
          description:
              'Your current tracking consistency is ${(consistency * 100).round()}%. '
    ...
    if (_hasNutritionCorrelation(cycles, profile)) {
      insights.add(
        PersonalizedInsight(
          ...
          description:
              'Based on your tracking patterns, certain symptoms appear linked to '
    ...
    if (cycleEvolution.isNotEmpty) {
      insights.add(
        PersonalizedInsight(
          ...
          description:
              'We\'ve detected gradual changes in your cycle patterns over time. '
  ...
  Future<List<PersonalizedRecommendation>> _generateLifestyleRecommendations(
    ...
    final recommendations = <PersonalizedRecommendation>[];
    final lifestyleRules =
        _personalizationRules['lifestyle_recommendations']
            as Map<String, dynamic>;

    // Exercise recommendations based on cycle phase
    final exerciseRules =
        lifestyleRules['exercise_suggestions'] as Map<String, dynamic>;
    if (exerciseRules.containsKey(currentPhase)) {
      ...
      recommendations.add(
        PersonalizedRecommendation(
          ...
          description:
              'Based on your $currentPhase phase, these activities align with your body\'s needs',
  ...
  Future<List<PersonalizedRecommendation>> _generateHealthRecommendations(
    ...
    if (currentCycle != null && currentCycle.symptoms.isNotEmpty) {
      final symptomBasedRecs = _getSymptomBasedRecommendations(
        currentCycle.symptoms,
        profile,
      );
  ...
  Future<List<PersonalizedRecommendation>> _generateBehavioralRecommendations(
    ...
    recommendations.add(
      PersonalizedRecommendation(
        ...
        description:
            'Small improvements to your tracking can significantly enhance AI predictions',
        actionItems: [
  ...
  double _analyzeStressImpactOnCycles(List<CycleData> cycles) {
    ...
    final stressCycles = cycles
        .where((c) => c.symptoms.contains('stress'))
        .length;
  ...
  double _calculateTrackingConsistency(List<CycleData> cycles) {
    ...
    final cyclesWithData = cycles
        .where(
          (c) => c.symptoms.isNotEmpty || c.mood != null || c.energy != null,
        )
        .length;
  ...
  bool _hasNutritionCorrelation(List<CycleData> cycles, UserProfile profile) {
    ...
    return profile.healthConcerns.contains('nutrition') ||
        cycles.any(
          (c) =>
              c.symptoms.contains('bloating') || c.symptoms.contains('cramps'),
        );
  }
  ...
  Map<String, String> _analyzeCycleEvolution(List<CycleData> cycles) {
    ...
    final recentAvg =
        recentCycles.map((c) => c.actualLength).reduce((a, b) => a + b) /
        recentCycles.length;
    final olderAvg =
        olderCycles.map((c) => c.actualLength).reduce((a, b) => a + b) /
        olderCycles.length;
    ...
    if ((recentAvg - olderAvg).abs() < 1) {
      ...
      return {
        ...
        'magnitude': (recentAvg - olderAvg).abs() > 3
            ? 'significant'
            : 'moderate',
      };
    }
  ...
  Map<String, dynamic> _analyzePersonalCyclePattern(
    List<CycleData> cycles,
    UserProfile profile,
  ) {
    // Analyze how user's cycles deviate from standard patterns
    final userAverage =
        cycles.map((c) => c.actualLength).reduce((a, b) => a + b) /
        cycles.length;
    final standardAverage = 28.0;
    return {'adjustment': userAverage - standardAverage};
  }

  Map<String, dynamic> _analyzePersonalSymptomPatterns(
    List<CycleData> cycles,
    UserProfile profile,
  ) {
  ...
  Map<String, dynamic> _analyzePersonalMoodEnergyPatterns(
    List<CycleData> cycles,
    UserProfile profile,
  ) {
    ...
    return {
      'mood_shift': moodData.isNotEmpty
          ? moodData.reduce((a, b) => a + b) / moodData.length - 3.0
          : 0.0,
      'energy_shift': energyData.isNotEmpty
          ? energyData.reduce((a, b) => a + b) / energyData.length - 3.0
          : 0.0,
    };
  }
    ...
    if (symptoms.contains('cramps')) {
      recommendations.add(
        PersonalizedRecommendation(
          ...
          description:
              'Based on your current symptoms, these approaches may help',
          actionItems: [
