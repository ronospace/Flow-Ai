...
class AIEngine {
  ...
  Future<void> initialize() async {
    ...
    _predictionModel = {
      'cycle_length_weights': [
        0.4,
        0.3,
        0.2,
        0.1,
      ], // Recent cycles weighted more
  ...
  Future<CyclePrediction> predictNextCycle(
    ...
    final fertilityPrediction = _calculateFertileWindow(
      lengthPrediction,
      historicalCycles,
    );

    final lastCycle = historicalCycles.last;
    final predictedStart = _calculateNextStartDate(
      lastCycle,
      lengthPrediction.predictedLength,
    );
  ...
  AIInsight? _analyzeSymptomPatterns(List<CycleData> cycles) {
    ...
    return AIInsight(
      ...
      description:
          'You experience ${mostCommon.key} in ${((mostCommon.value / cycles.length) * 100).round()}% of your cycles.',
      confidence: 0.8,
      actionable: true,
      recommendations: [
        'Track timing of ${mostCommon.key}',
  ...
  AIInsight? _analyzeHealthTrends(List<CycleData> cycles) {
    ...
    final recentAvgLength =
        recentCycles.map((c) => c.length).reduce((a, b) => a + b) /
        recentCycles.length;
    final olderAvgLength =
        olderCycles.map((c) => c.length).reduce((a, b) => a + b) /
        olderCycles.length;

    if ((recentAvgLength - olderAvgLength).abs() > 2) {
      return AIInsight(
        type: InsightType.healthTrend,
        title: recentAvgLength > olderAvgLength
            ? 'Cycles getting longer'
            : 'Cycles getting shorter',
        description:
            'Your recent cycles average ${recentAvgLength.toStringAsFixed(1)} days vs ${olderAvgLength.toStringAsFixed(1)} days previously.',
        confidence: 0.7,
        actionable: true,
        recommendations: [
          'Monitor for 2-3 more cycles',
  ...
  CycleLengthPrediction _predictCycleLength(List<CycleData> cycles) {
    ...
    final variance = _calculateVariance(
      completedCycles.map((c) => c.actualLength.toDouble()).toList(),
    );
    ...
    return CycleLengthPrediction(adjustedLength.round(), confidence, factors);
  }
  ...
  Future<MoodEnergyForecast> _forecastMoodEnergy(List<CycleData> cycles) async {
    ...
    final baselineMood = moodData.isNotEmpty
        ? moodData.reduce((a, b) => a + b) / moodData.length
        : 3.0;
    final baselineEnergy = energyData.isNotEmpty
    ...
    for (final phase in phases) {
      final phaseEffect =
          _moodEnergyModel['cycle_phase_effects'][phase]
              as Map<String, dynamic>;
      moodForecast[phase] = math.max(
        1.0,
        math.min(5.0, baselineMood + phaseEffect['mood']),
      );
      energyForecast[phase] = math.max(
        1.0,
        math.min(5.0, baselineEnergy + phaseEffect['energy']),
      );
    }
  ...
  Map<String, double> _predictPhaseSpecificSymptoms(List<CycleData> cycles) {
    ...
    final phaseSymptoms =
        _symptomModel['symptom_phases']['luteal'] as List<String>;
  ...
  double _calculateSymptomPredictionConfidence(List<CycleData> cycles) {
    final cyclesWithSymptoms = cycles
        .where((c) => c.symptoms.isNotEmpty)
        .length;
    return math.min(
      1.0,
      cyclesWithSymptoms / math.max(1, cycles.length).toDouble(),
    );
  }

  DateTime _calculateNextStartDate(CycleData lastCycle, int predictedLength) {
    if (lastCycle.isCompleted) {
      return lastCycle.endDate!.add(
        Duration(days: predictedLength - lastCycle.actualLength),
      );
    } else {
  ...
  FertileWindow _calculateFertileWindow(
    CycleLengthPrediction prediction,
    ...
    final adjustment = cycles.length >= 3
        ? _calculateOvulationAdjustment(cycles)
        : 0;
  ...
  Future<void> _initializeFlowAI() async {
    ...
    try {
      ...
      debugPrint(
        '🤖 AI Engine FlowAI integration ${_useFlowAI ? "enabled" : "failed"}',
      );
    } catch (e) {
  ...
  Map<String, dynamic> _prepareCycleAnalysisForFlowAI(List<CycleData> cycles) {
    ...
    analysis['length_variance'] = _calculateVariance(
      lengths.map((l) => l.toDouble()).toList(),
    );
          ...
          if (title.isNotEmpty && description.isNotEmpty) {
            insights.add(
              AIInsight(
                ...
                confidence:
                    0.85, // FlowAI insights are generally high confidence
                actionable:
                    description.contains('consider') ||
                    description.contains('try'),
  ...
  List<String> _extractRecommendations(String text) {
    ...
    if (text.contains('track')) {
      recommendations.add('Continue tracking your data');
    }
    if (text.contains('exercise')) {
      recommendations.add('Consider adjusting exercise routine');
    }
    if (text.contains('stress')) {
      recommendations.add('Focus on stress management');
    }
    if (text.contains('sleep')) {
      recommendations.add('Prioritize good sleep hygiene');
    }
    ...
    return recommendations.isNotEmpty
        ? recommendations
        : ['Keep monitoring your patterns'];
  }

  /// Check if local insight overlaps with FlowAI insights
  bool _hasOverlapWithFlowAIInsights(
    AIInsight localInsight,
    List<AIInsight> flowAIInsights,
  ) {
    ...
    for (final flowAIInsight in flowAIInsights) {
      ...
      final overlap = localKeywords
          .where((word) => flowAIKeywords.contains(word))
          .length;
  ...
  String get engineStatus {
    ...
    if (_useFlowAI && _flowAIService.isInitialized) {
      return 'Enhanced with FlowAI';
    }
