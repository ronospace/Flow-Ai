...
enum PredictionConfidence { high, medium, low, veryLow }
...
class CycleCalculationEngine {
  ...
  CyclePrediction? calculateNextCycle(List<CycleData> cycles) {
    ...
    return CyclePrediction(
      ...
      fertileWindow: FertileWindow(
        start: predictedStartDate.subtract(
          Duration(days: avgLength.round() - ovulationDay + 5),
        ),
        end: predictedStartDate.subtract(
          Duration(days: avgLength.round() - ovulationDay - 1),
        ),
        peak: predictedStartDate.subtract(
          Duration(days: avgLength.round() - ovulationDay),
        ),
      ),
    );
  }
  ...
  CycleStatistics _calculateCycleStatistics(List<CycleData> cycles) {
    ...
    for (int i = 0; i < cycles.length - 1; i++) {
      ...
      final length = nextCycle.startDate
  ...
  DateTime _predictNextPeriod(
    ...
    final estimatedEnd =
        currentCycle.endDate ?? _estimateCycleEnd(currentCycle);
    if (referenceDate.isBefore(estimatedEnd)) {
      // We're still in current cycle
      return currentCycle.startDate.add(
        Duration(days: stats.averageCycleLength),
      );
    }
  ...
  DateRange _calculateFertileWindow(
    DateTime nextPeriodDate,
    CycleStatistics stats,
  ) {
    // Ovulation typically occurs 14 days before next period
    final ovulationDate = nextPeriodDate.subtract(
      Duration(days: averageLutealPhase),
    );
  ...
  CyclePhase _calculateCurrentPhase(
    CycleData? currentCycle,
    DateTime referenceDate,
    CycleStatistics stats,
  ) {
    ...
    final daysSinceStart = referenceDate
        .difference(currentCycle.startDate)
        .inDays;
  ...
  PredictionConfidence _calculateConfidence(
    ...
    if (cycles.isNotEmpty) {
      final daysSinceLastCycle = DateTime.now()
          .difference(cycles.first.startDate)
          .inDays;
  ...
  CyclePredictions _getDefaultPredictions(DateTime referenceDate) {
    final nextPeriod = referenceDate.add(
      const Duration(days: averageCycleLength),
    );
    final ovulation = nextPeriod.subtract(
      const Duration(days: averageLutealPhase),
    );
