...
class ProgressiveDisclosureService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    for (final feature in _allFeatures) {
      _featureStates[feature] =
          prefs.getBool('$_featurePrefix$feature') ??
          _isFeatureUnlockedByDefault(feature);
    }
  ...
  Future<void> _checkAndUnlockFeatures() async {
    ...
    if (!isFeatureUnlocked(featureCycleCalendar) &&
        getActionCount(actionLogPeriod) >= 1) {
      await unlockFeature(featureCycleCalendar);
      unlocked = true;
    }

    // Unlock symptom tracking after 2 app uses
    if (!isFeatureUnlocked(featureSymptomTracking) &&
        getActionCount(actionUseApp) >= 2) {
      await unlockFeature(featureSymptomTracking);
      unlocked = true;
    }

    // Unlock mood tracking after 3 app uses
    if (!isFeatureUnlocked(featureMoodTracking) &&
        getActionCount(actionUseApp) >= 3) {
    ...
    final totalEntries =
        getActionCount(actionLogPeriod) +
    ...
    if (!isFeatureUnlocked(featureAdvancedAnalytics) &&
        getActionCount(actionViewInsights) >= 3) {
      await unlockFeature(featureAdvancedAnalytics);
      unlocked = true;
    }

    // Unlock AI Coach after 10 total actions
    final totalActions = _actionCounts.values.fold(
      0,
      (sum, count) => sum + count,
    );
    ...
    if (!isFeatureUnlocked(featureHealthDashboard) &&
        getActionCount(actionUseApp) >= 14) {
      await unlockFeature(featureHealthDashboard);
      unlocked = true;
    }

    // Unlock ML predictions after 3 complete cycles (90 days / 3 period logs)
    if (!isFeatureUnlocked(featureMlPredictions) &&
        getActionCount(actionLogPeriod) >= 3) {
      await unlockFeature(featureMlPredictions);
      unlocked = true;
    }

    // Unlock biometric integration after health dashboard is unlocked
    if (!isFeatureUnlocked(featureBiometricIntegration) &&
        isFeatureUnlocked(featureHealthDashboard)) {
      await unlockFeature(featureBiometricIntegration);
      unlocked = true;
    }

    // Unlock data export after 30 app uses
    if (!isFeatureUnlocked(featureDataExport) &&
        getActionCount(actionUseApp) >= 30) {
  ...
  List<FeatureUnlockRequirement> getAllFeatureRequirements() {
    return _allFeatures
        .map((feature) => getUnlockRequirement(feature))
        .toList();
  }
...
class FeatureUnlockRequirement {
  ...
  int get remainingProgress => isUnlocked
      ? 0
      : (requiredProgress - currentProgress).clamp(0, requiredProgress);
}
