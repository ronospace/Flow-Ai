...
class ExportDateRange {
  ...
  ExportDateRange({required this.startDate, required this.endDate});
...
class DataExportService {
          ...
          pw.TableHelper.fromTextArray(
            ...
            data: cycles.take(20).map((cycle) {
              return [
                DateFormat(
                  'MMM d, yyyy',
                ).format(DateTime.parse(cycle['start_date'])),
  ...
  Future<String?> _exportAsCSV(
    ...
    try {
      ...
      csvBuffer.writeln(
        'Start Date,End Date,Length (days),Flow Intensity,Notes',
      );
      ...
      for (final cycle in cycles) {
        ...
        final endDate = cycle['end_date'] != null
            ? DateFormat('yyyy-MM-dd').format(DateTime.parse(cycle['end_date']))
            : 'N/A';
      ...
      for (final track in tracking) {
        final date = DateFormat(
          'yyyy-MM-dd',
        ).format(DateTime.parse(track['date']));
