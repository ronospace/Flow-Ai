...
class FloatingAIChat extends StatefulWidget {
  ...
  const FloatingAIRepaintBoundary(child: child: Chat({
    super.key,
...
_messagesSub = _chatService.messagesStream.listen((messages) {
  if (!mounted) return;
  final snapshot = List.from(messages);
  WidgetsBinding.instance.addPostFrameCallback((_) {
    if (!mounted) return;
    setState(() {
      _messages
  ...
  void _toggleRepaintBoundary(child: child: Chat() {
    setState(() {
  ...
  Widget _buildChatArea(ThemeData theme) {
    return SizedBox.expand(
      child: Listener(
        ...
        child: RepaintBoundary(child: child: Chat(
          messages: _messages,
