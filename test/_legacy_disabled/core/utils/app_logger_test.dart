...
void main() {
  group('AppLogger Tests', () {
    ...
    test('should log warning messages', () {
      ...
      expect(
        () => AppLogger.warning('Warning about something'),
        returnsNormally,
      );
    });
    ...
    test('should log debug messages', () {
      ...
      expect(
        () => AppLogger.debug('Debug data: ${DateTime.now()}'),
        returnsNormally,
      );
    });
    ...
    test('should handle auth-specific logging', () {
      ...
      expect(
        () => AppLogger.auth('User signed in: test@example.com'),
        returnsNormally,
      );
    });
    ...
    test('should format messages consistently', () {
      ...
      expect(
        () => AppLogger.info('Message with object: ${{'key': 'value'}}'),
        returnsNormally,
      );
    });
  ...
  group('AppLogger Integration Tests', () {
    test('should work in different logging scenarios', () {
      ...
      // Warning scenarios
      expect(
        () => AppLogger.warning('⚠️ Low battery detected'),
        returnsNormally,
      );
      expect(
        () => AppLogger.warning('📶 Poor network connection'),
        returnsNormally,
      );
      ...
      expect(
        () =>
            AppLogger.error('💥 Unexpected error occurred', Exception('Test')),
        returnsNormally,
      );
    });
    ...
    test('should maintain consistent behavior across log levels', () {
      ...
      expect(
        () => AppLogger.error(testMessage, testException),
        returnsNormally,
      );
    });
  });
